...
#[cfg(test)]
mod tests {
    use super::BelScanner;

    /// 验证扫描器忽略 OSC 终止 BEL，只报告普通输出中的裸 BEL。
    /// 参数：无；返回：无，断言失败时由测试框架报告。
    #[test]
    fn bel_scanner_ignores_osc_terminator_and_detects_ground_bel() {
        let mut scanner = BelScanner::new();

        assert!(!scanner.scan(b"\x1b]0;title"));
        assert!(!scanner.scan(b"\x07"));
        assert!(scanner.scan(b"ready\x07"));
    }
}
